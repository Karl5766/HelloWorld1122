--------------------------------------------------------------------------------------------
--	player_system.lua
--  Description: player相关的frame, colli以及一些event函数
--	Author: Unknown
--	Modifiers:
--		Karl, 2020/6/8, 把使用的MixTable函数用相同功能的两个函数StdLib.LinearInterpolation以及
--			StdLib.ArrayLinearInterpolation代替了
--
--------------------------------------------------------------------------------------------
    ...
    ["frame.control"] = { 98, function(self, system)
        if self.__death_state == 0 then
            if not self.dialog then
                ...
                if self.__spell_flag and self.nextspell <= 0 and not lstg.var.block_spell then
                    system:spell()
                end
        ...
        if not (self.time_stop) then
            if self.slist then
                ...
                if self.support == 5 then
                    for i = 1, 4 do
                        self.sp[i] = StdLib.SingleArrayLinearInterpolation(self.slist[6][i], self.lh)
                        self.sp[i][3] = 1
                    end
                else
                    ...
                    for i = 1, 4 do
						local lowPowerList, highPowerList = self.slist[s][i], self.slist[s + 1][i]
                        if lowPowerList and highPowerList then
                            self.sp[i] = StdLib.ArrayLinearInterpolation( 
								StdLib.SingleArrayLinearInterpolation(lowPowerList, self.lh),
                                StdLib.SingleArrayLinearInterpolation(highPowerList, self.lh),
								t
							)
                            self.sp[i][3] = 1
                        elseif highPowerList then
                            self.sp[i] = StdLib.SingleArrayLinearInterpolation(highPowerList, self.lh)
                            self.sp[i][3] = t
                        end
                    end
                end
...
function system:spell()
	...
	if lstg.var.bomb > 0 then
		...
		-- Call player-dependent spell() method
...
function system:colli(other)
    ...
    if p.death == 0 and not p.dialog and not cheat then
        if p.protect == 0 then
			if other.stun and other.stun > 0 then
				p.stun = other.stun
			else
				if GetKeyState(KEY.C) and globalSoul >= 95 then
					p.death = 0
					lstg.var.deathrelease = 1
				else
					PlaySound("pldead00", 0.5)
					lstg.var.playerDied = 100
					p.death = 100
				end
			end
        end
...
function system:updateKeyState()
    ...
    for key in pairs(self._keys) do --更新已注册按键状态并执行事件组
    ...
    for key in pairs(self._keys_remove) do --移除解除注册的按键并执行应有的事件组
        if self.keyState[key] then
            self:doKeyEvent(key, "release") --抬起
            self:doKeyEvent(key, "up") --保持抬起（包括抬起）
        end
